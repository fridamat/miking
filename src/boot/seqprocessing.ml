...
let rec init_found_seqs_assoc_list rels_assoc_l =
  match rels_assoc_l with
  | [] -> []
  | (hd,_)::tl ->
    (hd,false)::(init_found_seqs_assoc_list tl)

let rec find_all_related seqs rels_assoc_l new_seqs rel_seqs found_seqs_assoc_l =
  match seqs, new_seqs with
  | [], [] -> (rel_seqs,found_seqs_assoc_l)
  | [], _ -> find_all_related new_seqs rels_assoc_l [] rel_seqs found_seqs_assoc_l
  | (hd::tl), _ ->
    if List.assoc hd found_seqs_assoc_l then
      find_all_related tl rels_assoc_l new_seqs rel_seqs found_seqs_assoc_l
    else
      let upd_found_seqs_assoc_l = upd_rels_assoc_list_bool_entry hd found_seqs_assoc_l true in
      let hd_rel_seqs = List.assoc hd rels_assoc_l in
      let upd_new_seqs = List.append hd_rel_seqs new_seqs in
      let upd_rel_seqs = hd::rel_seqs in
      find_all_related tl rels_assoc_l upd_new_seqs upd_rel_seqs upd_found_seqs_assoc_l

let rec reduce_rels seq_cons rels_assoc_l red_rels_assoc_l found_seqs_assoc_l =
  match seq_cons with
  | [] -> rels_assoc_l
  | hd::tl ->
    let (rel_seqs,upd_found_seqs_assoc_l) = find_all_related [hd] rels_assoc_l [] [] found_seqs_assoc_l in
    let upd_red_rels_assoc_l = (hd,rel_seqs)::red_rels_assoc_l in
    reduce_rels tl rels_assoc_l upd_red_rels_assoc_l upd_found_seqs_assoc_l
...
let process_ast ast =
  ...
  let rels_assoc_l3 = reduce_rels seq_cons rels_assoc_l2 [] (init_found_seqs_assoc_list rels_assoc_l2) in
  (*Continue here since we don't get the right rels*)
