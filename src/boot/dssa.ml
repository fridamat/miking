...
module Dssa = struct
  ...
  let get_new_best_max_cost_term_and_best_data_structures best_max_cost_term best_data_structures contender_cost_term contender_data_structure =
    ...
    | 0 -> (best_max_cost_term, (add_to_end_of_list best_data_structures contender_data_structure)) (*best_max_cost_term = contender_cost_term*) (*WC: O(#data structures)*)
      ...
      (match data_structures with
       ...
       | ds_hd::ds_tl ->
         ...
         if cost_term_count < min_count then
           ...
           let updated_contenders = add_to_end_of_list contenders ds_hd in (*We add the current data structure to the list of contenders*) (*WC: O(#data structures)*)
  ...
  let rec combine_complexities_with_frequency frequency complexities =
    ...
    | [] -> []
    | comp_hd::comp_tl -> (Costterms.create_cost_term comp_hd frequency)::(combine_complexities_with_frequency frequency comp_tl) (*O(1)*)
  ...
  let init_cost_record =
    let cost_terms = build_cost_terms Frequencies.get_frequencies Complexities.get_complexities in init_cost_record_helper cost_terms (*WC: O(#frequencies)*)
  ...
  let rec go_through_data_structures j mf_row_i mc best_max_cost_term best_data_structures costs =
    ...
    if j == (List.length mc) then (*WC: O(#data structures)*)
      ...
      let updated_costs = add_to_end_of_list costs cost_record_j in (*TODO: Make costs into an assoc list?*) (*WC: O(#data structures)*)
  ...
  let main mf =
    let mc = Sequenceinfo.get_mc in (*O(1)*)
